...
namespace NotesMarketPlace.Controllers
{
    ...
    public class NotesUnderReviewController : Controller
    {
        ...
        {
                List<SellerNotes> NoteTitlePublished = objAdmin.SellerNotes.Where(x => x.IsActive == true &&(x.Title.Contains(SearchUnderReview)||x.NoteCategories.Name.Contains(SearchUnderReview)||x.ReferenceData.Value.StartsWith(SearchUnderReview)
                || (x.ModifiedDate.Value.Day + "-" + x.ModifiedDate.Value.Month + "-" + x.ModifiedDate.Value.Year).Contains(SearchUnderReview)
                || x.Users.FirstName.Contains(SearchUnderReview)||x.Users.LastName.Contains(SearchUnderReview)||SearchUnderReview == null)).ToList();
    ...
    .Select(s => new
    {
        ...
        public ActionResult InReview(int? id)
        {
            ...
            TempData["AdminDashboard"] = user.FirstName + " " + user.LastName;
            TempData["title"] = note.Title;
            TempData["Message"] = " InReview Successfully !";
            return RedirectToAction("NotesUnderReview", "Admin");
        }
        ...
        public ActionResult RejectedNote(int? id, AdminRemarks adminremark)
        {
            ...
            TempData["AdminDashboard"] = user.FirstName + " " + user.LastName;
            TempData["title"] = note.Title;
            TempData["Message"] = " Rejected Successfully !";
            return RedirectToAction("NotesUnderReview", "Admin");
        }
