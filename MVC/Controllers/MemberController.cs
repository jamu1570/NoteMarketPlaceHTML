...
namespace NotesMarketPlace.Controllers
{
    ...
    public class MemberController : Controller
    {
        ...
        public ActionResult Dashboard(string SearchText, string SearchPublished, int? PublishedNotespage, int? ProgressNotespage, string SortOrderProgress, string SortOrderPublished)
        {
            ...
            List<SellerNotes> NoteTitle = objMember.SellerNotes.OrderByDescending(x => x.CreatedDate).Where(x => x.CreatedBy == userObj.UserID && x.IsActive == true && (x.Title.Contains(SearchText) || SearchText == null)).ToList();
            ...
            ViewBag.ProgressNote = ProgressNote.ToList().ToPagedList(ProgressNotespage ?? 1, 3);

            List<SellerNotes> NoteTitlePublished = objMember.SellerNotes.OrderByDescending(x => x.CreatedDate).Where(x => x.CreatedBy == userObj.UserID && x.IsActive == true && (x.Title.Contains(SearchPublished) || SearchPublished == null)).ToList();
        ...
        public ActionResult EditNote(int? id, AddNote objAddNote, string submit)
        {
            if (ModelState.IsValid)
            {
                ...
                if (submit == "Publish")
                {
                    ...
                    PublishedNoteEmailcs.PublishedNoteNotify(userObj, objAddNote.Title);
                }
                else if (submit == "Save")
                {
                    notevalue = "Draft";
        ...
        public ActionResult BuyerRequest(int? page, string SearchNoteTitle, string SortOrder)
        {
            ...
            List<Downloads> Download = objMember.Downloads.Where(x => x.Seller == userObj.UserID && x.IsSellerHasAllowedDownload == false && x.IsPaid == true && (x.NoteTitle.Contains(SearchNoteTitle) || x.NoteCategory.Contains(SearchNoteTitle) || x.PurchasedPrice.ToString().StartsWith(SearchNoteTitle) || SearchNoteTitle == null)).ToList();
        ...
        public ActionResult AllowDownload(int? id)
        {
            ...
            AllowDownloadEmail.AlloWDownloadNotifyEmail(SellerDetail, BuyerDetail);
        ...
        public ActionResult MyDownloads(int? page, string SearchNoteTitle, string SortOrder)
        {
            ...
            List<Downloads> Download = objMember.Downloads.Where(x => x.Downloader == userObj.UserID && x.IsSellerHasAllowedDownload == true && x.AttachmentPath != null && (x.NoteTitle.Contains(SearchNoteTitle) || x.NoteCategory.Contains(SearchNoteTitle) || x.PurchasedPrice.ToString().StartsWith(SearchNoteTitle) || SearchNoteTitle == null)).ToList();
            List<Users> user = objMember.Users.ToList();
            /*List<UserProfile> userprofile = objMember.UserProfile.ToList();*/
            /* (x.NoteTitle.Contains(SearchNoteTitle) || x.NoteCategory.Contains(SearchNoteTitle) || x.PurchasedPrice.ToString().StartsWith(SearchNoteTitle) || string.IsNullOrEmpty(SearchNoteTitle))*/
        ...
        public ActionResult RepotedIssues(int? id,ReportedIssues reportissues)
        {
           ...
           /* Downloads note = objMember.Downloads.Where(x=>x.DownloadsID == id).FirstOrDefault();*/
            ...
            ReportedIssuesEmail.ReportedIssuesNoteNotify(sellername, user, title);
        ...
        public ActionResult MySoldNotes(int? page, string SearchNoteTitle, string SortOrder)
        {
            ...
            List<Downloads> Download = objMember.Downloads.Where(x => x.Seller == userObj.UserID && x.IsSellerHasAllowedDownload == true && x.AttachmentPath != null && (x.NoteTitle.Contains(SearchNoteTitle) || x.NoteCategory.Contains(SearchNoteTitle) || x.PurchasedPrice.ToString().StartsWith(SearchNoteTitle) || SearchNoteTitle == null)).ToList();
            List<Users> user = objMember.Users.ToList();
            /*List<UserProfile> userprofile = objMember.UserProfile.ToList();*/
            /* (x.NoteTitle.Contains(SearchNoteTitle) || x.NoteCategory.Contains(SearchNoteTitle) || x.PurchasedPrice.ToString().StartsWith(SearchNoteTitle) || string.IsNullOrEmpty(SearchNoteTitle))*/
        ...
        public ActionResult MyRejectedNote(int? page, string SearchNoteTitle, string SortOrderProgress)
        {
            ...
            List<SellerNotes> NoteTitle = objMember.SellerNotes.Where(x => x.SellerID == userObj.UserID && x.IsActive == true && (x.Title.Contains(SearchNoteTitle) || x.AdminRemarks.Contains(SearchNoteTitle) || SearchNoteTitle == null)).ToList();
            List<NoteCategories> CategoryName = objMember.NoteCategories.Where(x=>x.Name.Contains(SearchNoteTitle) || SearchNoteTitle == null).ToList();
